...
if (typeof jQuery === "undefined") {
  throw new Error("Images loader requires jQuery");
}
...
function getOrientation(src) {
  ...
  while (offset < length) {
    if (view.getUint16(offset + 2, false) <= 8) -1;
...
function imageToFormat(
  ...
  if (img.width > maxWidth) width = maxWidth;

  if (img.height > maxHeight) height = maxWidth;
...
+(function ($) {
  ...
  ImagesLoader.prototype.btnChangeClick = function (evt, obj) {
    ...
    reader.onload = async function (e) {
      ...
      imageNew.onload = function () {
        ...
        if (!(height == 245 && width == 1920)) {
          $("#imageError").removeClass("d-none");
          $("#imageError").text(
            "Invalid image size. Featured image size should be 1920 x 245."
          );
          $("#image").val("");
          return;
        } else {
          ...
          $("#imagePreview").attr("src", imageNew.src);
        }
        ...
        if (filesLength > options.maxfiles)
          self.ShowProgress(
            self.barStyle.warning,
            options.errormaxfiles + ": " + options.maxfiles,
            2000
          );
        ...
        (function handle(i) {
          ...
          fileReader.onload = function (e) {
            ...
            var idx = self.AttachmentArray.map(function (file) {
  ...
  ImagesLoader.prototype.btnRemoveClick = function (evt, obj) {
    ...
    // Update main image
    if (
      self.AttachmentArray.length > 1 &&
      self.AttachmentArray[0].FileName == $div.data("id")
    ) {
    ...
    if (idx !== -1) self.AttachmentArray.splice(idx, 1);
  ...
  ImagesLoader.prototype.btnLeftClick = function (evt, obj) {
    ...
    var imgSource =
      "data:image/jpeg;base64," + self.AttachmentArray[idx].Base64;
  ...
  ImagesLoader.prototype.ShowProgress = function (style, msg, delay = 0) {
    ...
    if (delay > 0) self.$progress.delay(delay).fadeOut(options.fadeTime);
  };
  ...
  ImagesLoader.prototype.execJsFunction = function () {
    ...
    if (this.jsFunction != null) eval(this.jsFunction);
  };
  ...
  ImagesLoader.prototype.ApplyFileValidationRules = function (readerEvt) {
    ...
    if (self.CheckFileUnique(readerEvt.name) == false) {
      self.ShowProgress(
        self.barStyle.danger,
        options.errorduplicate + " (" + readerEvt.name + ")"
      );
      return false;
    }
    ...
    // To check files count according to upload conditions
    if (self.CheckFilesCount() == false) return false;
  ...
  ImagesLoader.prototype.CheckFileUnique = function (filename) {
    ...
    for (var i = 0; i < self.AttachmentArray.length; i++)
      if (self.AttachmentArray[i].FileName == filename) return false;
  ...
  ImagesLoader.prototype.CheckValidity = function () {
    ...
    if (self.AttachmentArray.length < options.minSelect) {
      self.ShowProgress(
        self.barStyle.danger,
        options.errorminfiles + ": " + options.minSelect
      );
      retVal = false;
    }
  ...
  ImagesLoader.prototype.LoadImages = function () {
    ...
    (function handle(i) {
      if (i >= self.imagesToLoad.length) return;
  ...
  //Render attachments thumbnails.
  ImagesLoader.prototype.RenderThumbnail = function (
    src,
    name,
    main = false,
    reload = false
    ...
    if (reload) $ratioBox.attr("title", src.substring(src.indexOf("_") + 1));
    else $ratioBox.attr("title", decodeURI(name));
      ...
      .popover({
        ...
        title: function () {
          return (
            ...
            options.modifyimagetext +
            "</p>"
          );
        },
        content: function () {
          var popoverContent = $element
            .find('[data-type="popover"]')
            .first()
            .clone(true, true);
          ...
          if (idx == 0) {
            ...
            $(popoverContent)
              .find('[data-operation="left"]')
              .removeAttr("data-operation");
            $(popoverContent)
              .find('[data-operation="main"]')
              .addClass("disabled");
            $(popoverContent)
              .find('[data-operation="main"]')
              .css("cursor", "default");
  ...
  ImagesLoader.prototype.init = function () {
    ...
    this.$model.bind("remove", function (evt) {
      self.btnRemoveClick(evt, this);
    });
    this.$model.bind("main", function (evt) {
      self.btnMainClick(evt, this);
    });
    this.$model.bind("left", function (evt) {
      self.btnLeftClick(evt, this);
    });
    this.$model.bind("right", function (evt) {
      self.btnRightClick(evt, this);
    });
    ...
    this.$btnAdd.click(function (evt) {
      self.btnAddClick(evt, this);
    });

    this.$noImage = $element.find('[data-type="noimage"]');
    this.$noImage.click(function (evt) {
      self.btnAddClick(evt, this);
    });

    this.$btnRemove = $element.find('[data-type="remove"]');
    this.$btnRemove.click(function (evt) {
      self.btnRemoveClick(evt, this);
    });
    ...
    this.$files.bind("change", function (evt) {
      self.btnChangeClick(evt, this);
    });
    ...
    // Load images
    if (this.imagesToLoad != null) this.LoadImages();

    // Show add image
    if (
      this.imagesToLoad == null ||
      this.imagesToLoad.length < this.options.maxfiles
    )
      this.$addImage.show();
  };
  ...
  $.fn.imagesloader.loadCSS = function () {
    var href = $("script[src*='" + imagesloader_fileName + "']")
      .attr("src")
      .split("-")[0];
